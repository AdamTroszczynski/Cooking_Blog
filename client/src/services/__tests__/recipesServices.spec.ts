...
describe("saveRecipe()", () => {
  it("should return Recipe object if input is correct", async () => {
    ...
    const expectedResult = new Recipe(1,"testName",10,"testLevel","testType",1,[new Step(1, "testContent")],[new Ingredient(1, "testName", "testQua")],"testImage.jpg",0);

    mock
      .onPost(`${RECIPE_API_URL}/createRecipe`)
      .reply(201, response);
...
describe('getSingleRecipe()', () => {
  it('should return Recipe object if input is correct', async () => {
    ...
    const expectedResult = new Recipe(1,'testName',10,'testLevel','testType',1,[new Step(0,'testContent')],[new Ingredient(1,'testName','testQua')],'testImage.jpg',0);

    mock.onGet(`${RECIPE_API_URL}/singleRecipe/${recipeId}`).reply(201,response);
...
describe('getRecipesPage()', () => {
  it("should return user's array of Recipe objects if input is correct", async () => {
    ...
    const expectedResult = [new Recipe(2,'testName','testDate','testLevel','testType',1,[new Step(1,'testContent')],[new Ingredient(1,'testName','testQua')],'testImage.jpg',0)];

    mock.onGet(`${RECIPE_API_URL}/recipesPageUser?lastId=${lastId}&dishTypeId=${dishTypeId}&limit=${limit}&userId=${userId}`).reply(201, response);
  ...
  it("should return array of Recipe objects if input is correct", async () => {
    const lastId = 1;
    const dishTypeId = 1;
    const limit = 10;
    const response = [{
    ...
    mock.onGet(`${RECIPE_API_URL}/recipesPage?lastId=${lastId}&dishTypeId=${dishTypeId}&limit=${limit}`).reply(201, response);
    const result = await getRecipesPage(lastId,dishTypeId,limit);
  ...
  it('should return an Error if input is incorrect and isUser is false', () => {
    ...
    mock.onGet(`${RECIPE_API_URL}/recipesPage?lastId=${lastId}&dishTypeId=${dishTypeId}&limit=${limit}`).reply(401);
  ...
  it('should return an Error if input is incorrect and isUser is true', () => {
    ...
    mock.onGet(`${RECIPE_API_URL}/recipesPageUser?lastId=${lastId}&dishTypeId=${dishTypeId}&limit=${limit}&userId=${userId}`).reply(401);
